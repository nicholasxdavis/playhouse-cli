...
fn render_engines(f: &mut Frame, sub: Rect, body: Rect, footer: Rect) {
    ...
    let lines = vec![
        ...
        Line::from(vec![Span::styled("  playhouse playwright [pattern] --json", theme::accent()), Span::styled(" - Playwright E2E only", theme::text())]),
        Line::from(vec![Span::styled("  playhouse upgrade|update --json", theme::accent()), Span::styled(" - check / apply releases (TUI: /upgrade, /update)", theme::text())]),
