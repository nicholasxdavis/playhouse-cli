    ...
    let exit_code = match cli.command {
        ...
        Some(Commands::Agent { action }) => match action {
            ...
            Some(AgentAction::Handoff { url }) => {
                cli::run_agent_handoff(&workspace, url.as_deref(), json, &settings).await
            }
        }
        ...
        Some(Commands::Lighthouse { url }) => {
            let target = cli::resolve_url(&workspace, url, &settings);
        ...
        Some(Commands::Verify { url }) => {
            ...
            cli::run_verify(&workspace, target.as_deref(), json, &settings).await
        }
                ...
                match std::fs::read_to_string(&path) {
                    Ok(content) => {
                        if json {
                            ...
                            cli::print_last_score(&path, &content);
                        }
                        0
                    }
                    Err(_) => {
